...
* ----- ---- -------- -----------------------------------------------
* 1.00a	rhk	12/22/14	First release of test program.  Builds on the n4_testpwm for the n4eif peripheral
* </pre>
*
* @note
...
volatile u32 			software_detector_high;
...
volatile unsigned int		detected_pwm_duty;
volatile unsigned int 		detected_pwm_freq;
...
int do_init(void)
{
	...
	status = XGpio_Initialize(&GPIOInst0, GPIO_DEVICE_ID);
	if (status != XST_SUCCESS)
	{
		return XST_FAILURE;
	}
	...
	// initialize the GPIO instance
		status = XGpio_Initialize(&GPIOInst1, GPIO_DEVICE_ID);
		if (status != XST_SUCCESS)
		{
			return XST_FAILURE;
		}
...
void FIT_Handler(void)
{
	...
	hardware_detector_high = XGpio_DiscreteRead(&GPIOInst1, 1);
	hardware_detector_low = XGpio_DiscreteRead(&GPIOInst1, 2);
	//if (gpio_in & 0x00000001)
	//if (hardware_detector_high==hardware_detector_low)
	//{
	...
	///here we will take the output of either the hardware or the software pwm count and do math on it.
	//for now, i will light up leds, since i dont know how to get at the lcd very well

	//I am also going to stick the software counter here, just cause
	...
	if (software_pwm_down_count < software_pwm_up_count)
	{
	...
	detected_pwm_duty = high_count/(high_count+low_count);
	detected_pwm_freq = 1000000000/(high_count+low_count);

	if (detected_pwm_duty > 50 )
		{
			NX4IO_setLEDs(0x0000FFFF);
			}
		else if (detected_pwm_duty <= 50)
		{
			NX4IO_setLEDs(0x00000000);
			}
